...
public class AsyncAuthToken extends AsyncTask<Void, Void, Void> {
	private static final String TAG = AsyncAuthToken.class.getName();
	...
	Account getAccountFromPrefs() {
		...
		if (sp.contains(SelectAccount.DEFAULT_ACCOUNT_KEY)) {
			String def = sp.getString(SelectAccount.DEFAULT_ACCOUNT_KEY, null);
			if (def != null) {
				AccountManager am = AccountManager.get(context);
				Account[] accounts = am.getAccountsByType(Main.ACCOUNT_TYPE);
	...
	protected Void doInBackground(Void... params) {
		...
		if (account != null) {
	    	...
	    	AccountManagerFuture<Bundle> future =
	    		am.getAuthToken(account, APP_ENGINE, true, callback, handler);
	    	try {
